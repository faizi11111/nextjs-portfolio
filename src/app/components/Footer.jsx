...
const footerData = [
  ...
  },
  {
    ...
    items: [
      { name: CONTACT_INFO.phone, href: `tel:${CONTACT_INFO.phone}`, icon: "📱" },
      { name: CONTACT_INFO.email, href: `mailto:${CONTACT_INFO.email}`, icon: "✉️" },
      { name: CONTACT_INFO.location, href: null, icon: "🌐" },
      { 
        name: "LinkedIn", 
  ...
  return (
    <div className="border-b border-theme py-4 md:border-none">
      ...
      >
        <ul className="space-y-4">
          {section.items.map((item, index) => (
            <li key={index} className={`${item.icon ? "flex items-center" : ""} ${item.mobileOnly ? "md:hidden" : ""}`}>
      ...
      <div className="hidden md:block mt-6">
        <ul className="space-y-4">
          {section.items.filter(item => !item.mobileOnly).map((item, index) => (
